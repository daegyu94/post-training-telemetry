...
Controller에서 실행합니다.
이미 checkout이 있다면 미완료 작업을 보존하고 통합된 `main`의 코드를 사용하세요.
...
Controller와 Spark는 CPU architecture가 다르므로 Python 환경을 공유하지 않습니다.
각 Spark 노드의 로컬 디스크에 환경을 준비하세요.
두 Spark 노드에 각각 SSH로 접속해 다음을 확인합니다.

```bash
cd /home/spark/shared/post-training-lab/observability
hostname
uname -m
free -h
nvidia-smi
...
./scripts/install_spark_tools.sh server
# spark2: Node Exporter만 설치
./scripts/install_spark_tools.sh node
```
...
DURATION=1800 ./scripts/run_spark_observability.sh node
```
...
timeout 1800 ./scripts/run_spark_observability.sh server
```
...
./scripts/run_spark_profile.sh collective
```

반드시 두 launcher를 모두 시작하세요.
한쪽만 실행하면 상대 노드를 기다립니다.
...
```bash
./scripts/install_spark_tools.sh fio
...
OUTPUT_DIR="artifacts/spark/$PROFILE_RUN_ID/fio-local" ./scripts/run_fio_baseline.sh
export FIO_DIRECTORY="$PWD/artifacts/spark/$PROFILE_RUN_ID/fio-nfs-data"
OUTPUT_DIR="artifacts/spark/$PROFILE_RUN_ID/fio-nfs" ./scripts/run_fio_baseline.sh
```

이 예시는 256 MiB 파일에 4 MiB 단위로 직접 I/O를 수행하는 짧은 저장소 검사입니다.
Queue depth는 16이며 쓰기와 읽기를 각각 5초 동안 측정합니다.
...
```bash
./scripts/run_spark_profile.sh baseline
...
서로 다른 노드의 timestamp만으로 정밀한 rank 간 시간 차이를 단정하지 않습니다.
Manifest에 기록된 NTP 시간 동기화 여부도 확인하세요.
